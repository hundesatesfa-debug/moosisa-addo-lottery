...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: dict.admin.titles.activity };
}
  ...
  return (
    <AdminShell name={profile.full_name ?? dict.common.superAdmin} role={profile.role}>
      {logs && logs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        {actor?.full_name ?? dict.common.system} · {log.entity_type} {log.entity_id ? `· ${log.entity_id.slice(0, 8)}` : ""}
                      </p>
                    </div>
                  </div>
        ...
        <EmptyState
          icon={ScrollText}
          title={dict.admin.noActivityTitle}
          description={dict.admin.noActivityText}
        />
      )}
    </AdminShell>
  );
