...
import { getDictionary, getLocaleFromValue, t } from "@/i18n/config";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: dict.admin.titles.draw };
}
        ...
        {alreadyDrawn ? (
          ...
          <Card>
            <CardContent className="p-8 text-center">
              <h2 className="text-lg font-bold text-slate-900">{dict.admin.drawNotAvailable}</h2>
              <p className="mt-2 text-sm text-slate-500">
                {t(dict.admin.drawNotAvailableText, { status: lottery.status })}
              </p>
            </CardContent>
          </Card>
        )}
