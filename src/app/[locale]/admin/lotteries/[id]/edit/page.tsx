...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: dict.admin.titles.edit };
}
    ...
    <AdminShell name={profile.full_name ?? dict.common.admin} role={profile.role}>
      <Card className="mx-auto max-w-2xl">
        <CardContent className="p-6">
          <h2 className="text-lg font-bold text-slate-900">{dict.admin.editLottery}</h2>
          {!editable && (
            <p className="mt-2 text-sm font-medium text-red-600">
              {dict.admin.completedLocked}
            </p>
          )}
          <form action={updateLotteryAction} className="mt-4 space-y-4">
            ...
            <div>
              ...
              <Input id="max_ticket_number" name="max_ticket_number" type="number" defaultValue={Number(lottery.max_ticket_number)} min={1} disabled={!editable} />
            </div>
            <div>
              <Label htmlFor="max_ticket_number">{dict.admin.maxTicketNumber}</Label>
              <Input id="max_ticket_number" name="max_ticket_number" type="number" defaultValue={Number(lottery.max_ticket_number)} min={1} disabled={!editable} />
            ...
            {editable && (
              <Button type="submit" className="w-full">{dict.admin.saveChanges}</Button>
            )}
          </form>
        </CardContent>
      </Card>
