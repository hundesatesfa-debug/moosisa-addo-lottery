      ...
      {winners && winners.length > 0 ? (
        <div className="space-y-3">
          {winners.map((w) => {
            ...
            const t = w.tickets as unknown as { ticket_code?: string };
            return (
              <Card key={w.id}>
                <CardContent className="flex flex-wrap items-center gap-4 p-5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-slate-900">
                      #{w.position} — {owner.full_name ?? "Anonymous"}
                    </p>
                    <p className="text-xs text-slate-400">
                      {lot.title} · {owner.phone ?? ""} · {t.ticket_code}
                    </p>
                  </div>
        ...
        <EmptyState
          ...
          description="Winners appear here after a draw is completed."
        />
      )}
    </AdminShell>
  );
